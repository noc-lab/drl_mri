...
def PN_process(filename,target_num):
	...
	random.seed(3) # try to use the same sample pairs for P and N sets. seems like it can avoid low rank issue
  ...
  #print(data)
	...
	print(results.shape)
	io.savemat(filename+'.mat', {'data': results.astype(np.float32)}) # be careful, must save as float (double), or matlab will fail!!!
...
if seq_len==1:
  io.savemat(diff_fn+'.mat', {'data': diff_data.astype(np.float32)})
else:
